...
import 'package:extended_http_client/src/ntlm/des/des.dart';
...
void _oddParity(List<int> bytes) {
  for (var i = 0; i < bytes.length; i++) {
    ...
    final needsParity = (((b >> 7) ^ (b >> 6) ^ (b >> 5) ^ (b >> 4) ^ (b >> 3) ^ (b >> 2) ^ (b >> 1)) & 0x01) == 0;
...
KeyParameter _createDESKey(List<int> bytes, int offset) {
  ...
  final keyBytes = List<int>.generate(7, (i) => uint8Bytes[i + offset]);
  final material = List<int>.filled(8, 0);
...
Uint8List encodeUtf16le(String s) => Uint8List.fromList(
      s.codeUnits.expand((codeUnit) => [codeUnit & 0xFF, codeUnit >> 8]).toList(),
    );

Uint8List createLMHashedPasswordV1(String password) {
  final oemPassword = ascii.encode(password.toUpperCase());
  final length = math.min(oemPassword.length, 14);
  final keyBytes = List<int>.filled(14, 0);
  _arrayCopy(oemPassword, 0, keyBytes, 0, length);
  final lowKey = _createDESKey(keyBytes, 0);
  final highKey = _createDESKey(keyBytes, 7);
  final magic = ascii.encode('KGS!@#\$%');
  final BlockCipher des = ECBBlockCipher(DESEngine());
  des.init(true, lowKey);
  final lowHash = des.process(magic).map((v) => v.toSigned(8)).toList();
  des.init(true, highKey);
  final highHash = des.process(magic).map((v) => v.toSigned(8)).toList();
  final lmHash = Uint8List(16);
...
Uint8List calculateResponse(Uint8List hash, Uint8List challenge) {
  final keyBytes = List.filled(21, 0);
  _arrayCopy(hash, 0, keyBytes, 0, 16);
  final lowKey = _createDESKey(keyBytes, 0);
  final middleKey = _createDESKey(keyBytes, 7);
  final highKey = _createDESKey(keyBytes, 14);
  final BlockCipher des = ECBBlockCipher(DESEngine());
  final uint8Challenge = Uint8List.fromList(challenge);
  des.init(true, lowKey);
  final lowResponse = des.process(uint8Challenge).map((v) => v.toSigned(8)).toList();
  des.init(true, middleKey);
  final middleResponse = des.process(uint8Challenge).map((v) => v.toSigned(8)).toList();
  des.init(true, highKey);
  final highResponse = des.process(uint8Challenge).map((v) => v.toSigned(8)).toList();
  final lmResponse = Uint8List.fromList(List.filled(24, 0));
...
Uint8List calculateLMResponseV2(
  ...
  final ntHash2 = createNTHashedPasswordV2(username, ntPassword);
  final hmac = HMac(MD5Digest(), 64);
  ...
  final hashed = hmac.process(buf.buffer.asUint8List(8));
...
Uint8List calculateNTLMResponseV2(
  ...
  final buf = ByteData(48 + (msg2.targetInfo?.length ?? 0));
  final ntHash2 = createNTHashedPasswordV2(username, ntPassword);
  final hmac = HMac(MD5Digest(), 64);
  hmac.init(KeyParameter(ntHash2));
  ...
  final timestamp = (DateTime.now().millisecondsSinceEpoch + 11644473600000) * 10000;
  ...
  // Zero
  buf.setUint32(44 + (msg2.targetInfo?.length ?? 0), 0, Endian.little);
...
Uint8List createRandomNonce([int length = 8]) => Uint8List.fromList(List.generate(length, (i) => _random.nextInt(255)));
