// ignore_for_file: constant_identifier_names, parameter_assignments, avoid_multiple_declarations_per_line

import 'dart:typed_data';

import 'package:extended_http_client/src/ntlm/des/des_constants.dart';
...
class DESEngine implements BlockCipher {
  ...
  List<Int32> _generateWorkingKey(bool encrypting, Uint8List key) {
    final newKey = List<Int32>.filled(32, Int32.ZERO);
    final pc1m = List<bool>.filled(56, false);
    final pcr = List<bool>.filled(56, false);
...
void pack32(int x, TypedData out, int offset, Endian endian) {
  assert((x >= 0) && (x <= _MASK_32));
  if (out is! ByteData) {
    out = ByteData.view(out.buffer, out.offsetInBytes, out.numberOfElements);
  }
...
/// [ByteData] if you will run it several times against the same buffer and want faster execution.
int unpack32(TypedData inp, int offset, Endian endian) {
  if (inp is! ByteData) {
    inp = ByteData.view(inp.buffer, inp.offsetInBytes, inp.numberOfElements);
  }
  return inp.getUint32(offset, endian);
}

extension on TypedData {
  int get numberOfElements => lengthInBytes ~/ elementSizeInBytes;
}
